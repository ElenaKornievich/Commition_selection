...
public class FacultyInformationCommand  implements BaseCommand{
    ...
    public String getPage(HttpServletRequest request) {
        ...
        try {
           ArrayList<FacultySubject> facultySubject= FacultySubjectsService.getInstance().findFacultySubjectsByFacultyId(facultyId);
           ArrayList<String> facultySubjectName=new ArrayList<>();
           for(FacultySubject var: facultySubject){
               facultySubjectName.add(SubjectService.getInstance().findSubjectById(var.getSubjectId()).getName());
           }
           request.setAttribute("facultySubjectName", facultySubjectName);
           request.setAttribute("listSubjects", SpecialityService.getInstance().findSpecialityByFacultyId(facultyId));
           request.setAttribute(AttributeConstant.ATTRIBUTE_FACULTY, FacultyService.getInstance().findFacultyById(facultyId));
